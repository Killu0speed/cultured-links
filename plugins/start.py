...
from database.database import add_user, del_user, full_userbase, present_user, is_admin, get_cached_invite, save_invite_link
...
async def start_command(client: Bot, message: Message):
    ...
    if len(text) > 7:
        try:
            ...
            # First check old link if no then new Mind Fumk..
            cached_invite_link = await get_cached_invite(channel_id, is_request)

            if cached_invite_link:
                invite_link = cached_invite_link
            else:
                ...
                await save_invite_link(channel_id, invite_link, is_request)
                asyncio.create_task(revoke_invite_after_10_minutes(client, channel_id, invite_link, is_request))
        ...
        await message.reply_text(
            "<b><i>Welcome to our smart link bot! 🔗 Share freely</i></b>",
...
async def help_callback(client: Bot, callback_query):
    ...
    await callback_query.message.edit_text("<b>1. First Join the channel \n2. Tap on Original link again or Reload ⚡️ \n3. Tap on Start and Done ✅.</b>", reply_markup=inline_buttons)
